...
### 1.1 目的
本システムは、GitHub Actionsを活用して主要日本株10銘柄の株価データ（日足・15分足）および主要テクニカル指標を完全自動で収集・計算・蓄積するサーバーレスデータパイプラインである。
...
2. **テクニカル指標の事前計算 (Pre-Calculated Indicators)**:
   - MA(5/25/75/200), RSI(14), MACD(12,26,9), ボリンジャーバンド(20, ±2σ), ATR(14), 52週高値(過去252日High最大値, confidenceフラグ) をPython（pandas/numpy）側で厳密に計算してCSV列に格納。
...
│   └── fetch_intraday.py            # 【Python】15分足データ取得・個別上書き
│
├── config/
...
| `week52_high` | Number | `3280` | `rolling(252).max()` | 過去252営業日（当日含む）の日中高値(`high`)の最大値 |
| `week52_high_confidence` | String | `full` | `full` / `partial` | 252営業日以上のデータ充足フラグ |
...
| `week52_high` | Number | `3280` | 52週高値（円、過去252日High最大値） |
| `week52_high_confidence` | String | `full` | 信頼度フラグ（`full` / `partial`） |